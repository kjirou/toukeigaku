...
/*::
export type DecimalConvertiableValue = number | string;
 ...
 */
const create度数分布 = (
  データセット/*: DecimalConvertiableValue[]*/,
  全階級の最小値/*: DecimalConvertiableValue*/,
  全階級の最大値/*: DecimalConvertiableValue*/,
  階級数/*: number*/,
  options/*: {
    相対度数の丸め桁数?: number,
...
const calculate度数分布の平均値 = (度数分布_/*: 度数分布*/)/*: DecimalValue*/ => {
  const データ数 = 度数分布_.reduce((m, 階級) => m + 階級.度数, 0);
  // reduce の memo 値が Decimal インスタンスだと Flow が落ちたので for で書いた
  let 合計 = new Decimal(0);
  for (let i = 0; i < 度数分布_.length; i++) {
    const 階級 = 度数分布_[i];
    合計 = 合計.plus(new Decimal(階級.階級値).mul(階級.度数));
  }
  return 合計.div(データ数).toString();
};
