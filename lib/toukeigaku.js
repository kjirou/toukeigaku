 ...
 * @param 階級数
 */
const create度数分布 = (データリスト, 全階級の最小値, 全階級の最大値, 階級数) => {
  ...
  for (let i = 0; i < 階級数; i++) {
    ...
    累積度数 = 累積度数.plus(度数);
    const 相対度数 = new Decimal(度数).div(データリスト.length);
    ...
    度数分布.push({
      ...
      階級値: (階級の最大値).minus(階級の最小値).div(2).plus(階級の最小値).toString(),
      度数,
      累積度数: 累積度数.toString(),
      相対度数: 相対度数.toString(),
      累積相対度数: 累積相対度数.toString(),
    });
  }
